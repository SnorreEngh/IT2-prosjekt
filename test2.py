...
for url in urls:
    ...
    try:
        ...
        company = soup.find('h1', {'class': 
          ...
          mobile:mb-2'}).text
        price = soup.find('div', {'class': 'instrument-price_instrument-price__xfgbB flex \items-end flex-wrap font-bold'}).find_all('span')
        change = soup.find('div', {'class': 'instrument-price_instrument-price__xfgbB\ flex items-end flex-wrap font-bold'}).find_all('span')
